...
run_migrations() {
    ...
    docker exec -i cryptoviz-timescaledb psql -q -U postgres -d cryptoviz < database/04-backfill-tracking.sql

    # News deduplication (unique URL constraint)
    docker exec -i cryptoviz-timescaledb psql -q -U postgres -d cryptoviz < database/05-news-dedup.sql
